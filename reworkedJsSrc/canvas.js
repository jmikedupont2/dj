...
((g, l) => l(g))(window, g => {
    ...
    g.new_image = function (a, c, b) {
        // Ensure `a` is a canvas element
        let canvas = document.getElementById(a); // Select the canvas element by ID
        if (!canvas) {
            console.error(`Canvas element with ID '${a}' not found.`);
            return null;
        }
        ...
        if (!ctx) {
            console.error(`Failed to get 2D context for canvas '${a}'.`);
            return null;
        }

        // Fill the canvas with white
        p(ctx, 0, 0, c, b, '#ffffff');
    ...
    g.draw_pixels = function (a) {
        let c = a.canvas.width, b = a.getImageData(0, 0, c, a.canvas.height); Object.keys(k).forEach(d => {
            let e = q(d); k[d].forEach(f => {
                f = f[1] * c + f[0] << 2; let h; for (h = 0; 4 > h; h++)
                    b.data[f + h] = e[h];
            });
        }); a.putImageData(b, 0, 0); n = !1; k = {};
    };
