...
'use strict';
const { gen_data } = require('./gen_data'); // Adjust the path as needed
const { $H, na } = require('./dungeon')
...
function select_from(a) { return a.constructor == Array ? select_from_array(a) : select_from_table(a); }
function select_from_array(a) { return a[random(a.length)]; }
function select_from_table(a) {
    var c; if (c = scale_table(a)) {
        c = random(c) + 1;
        let b;
        for (b in a) {
            let d = key_range(b);
...
function scale_table(a) {
    let c = 0, b; for (b in a)
        a = key_range(b), a[1] > c && (c = a[1]); return c;
}
function key_range(a) { let c; return (c = /(\d+)-00/.exec(a)) ? [parseInt(c[1], 10), 100] : (c = /(\d+)-(\d+)/.exec(a)) ? [parseInt(c[1], 10), parseInt(c[2], 10)] : '00' == a ? [100, 100] : [parseInt(a, 10), parseInt(a, 10)]; }
function new_trace() { return { exclude: {}, 'var': {} }; }
function local_trace(a) { let c = Object.clone(a); c['var'] = Object.clone(a['var']); return c; }
function expand_tokens(a, c) {
    let b = /\${ ([^{}]+) }/; for (var d; d = b.exec(a);) {
        d = d[1];
        let e;
...
function expand_x(a, c, b) {
    let d = {}, e = {}, f = [], l = b.comma || ', '; for (; match = /^(and|literal|unique) (.+)/.exec(c);)
        d[match[1]] = !0, c = match[2]; let k; for (k = 0; k < a; k++) {
            var g = new String(c);
            g = d.unique ? expand_unique(g, b) : expand_token(g, b);
            d.literal ? f.push(g) : (match = /^(\d+) x (.+)/.exec(g)) ? e[match[2]] += parseInt(match[1], 10) : e[g] += 1;
        } $H(e).keys().sort().forEach(h => { 1 < e[h] ? f.push([e[h], h].join(' x ')) : f.push(h); }); return d.and ? (a = f.pop(), f.length ? [f.join(l), a].join(' and ') : a) : f.join(l);
}
function expand_unique(a, c) {
    let b; for (b = 0; 100 > b; b++) {
...
function aoran(a) { return /^the /i.test(a) ? a : /^(nunchaku)/i.test(a) ? a : /^(unicorn|unique|university)/i.test(a) ? `a ${a}` : /^(hour)/i.test(a) ? `an ${a}` : /^[BCDGJKPQTUVWYZ][A-Z0-9]+/.test(a) ? `a ${a}` : /^[AEFHILMNORSX][A-Z0-9]+/.test(a) ? `an ${a}` : /^[aeiou]/i.test(a) ? `an ${a}` : `a ${a}`; }
function nothe(a) { let c; return (c = /^the (.+)/i.exec(a)) ? c[1] : a; }
function lc(a) { return a.toLowerCase(); }
function lcfirst(a) { let c; return (c = /^([a-z])(.*)/i.exec(a)) ? lc(c[1]) + c[2] : a; }
function inline_case(a) { return /^[A-Z][A-Z]/.test(a) ? a : lcfirst(a); }
function lthe(a) { let c; return (c = /^the (.+)/i.exec(a)) ? `the ${c[1]}` : a; }
function uc(a) { return a.toUpperCase(); }
function ucfirst(a) { let c; return (c = /^([a-z])(.*)/i.exec(a)) ? uc(c[1]) + c[2] : a; }
function title_case(a) { return a.split(/\s+/).map(uc).join(' '); }
;
module.exports = {
    generate_text
};